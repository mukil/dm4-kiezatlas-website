	...
	public MapEntryView(Topic geoObject, GeomapsService geomaps) {
		...
		if (addressTopic != null) {
			...
			if (geoCoordTopic != null) {
				...
				log.log(Level.WARNING, "**** KiezatlasEntry's Address ({0}) has no coordinate set!",
				    addressTopic.getSimpleValue());
			}
		} else {
	...
	public JSONObject toJSON() {
		try {
			...
			if (geoCoordinate == null) {
				log.log(Level.WARNING, "KiezatlasEntry has no geo-coordinate,"
				    + "NULL-ifying this object during serialization");
			} else {
				long aggregated_address_id = addressTopic.getChildTopics()
				    .getTopic("dm4.contacts.street").getId() +
